...
class VkClient:
    ...
    __slots__ = ("token", "session", "req_kwargs", "auth",
                 ...
                 "logger", "group_id", "user_id",
                 "queue", "loop")

    def __init__(self, proxy=None, logger=None, loop=None):
        if logger:
            self.logger = logger
        else:
    ...
    async def method(self, key, **data):
        ...
        if key == "messages.send" and "random_id" not in data.keys():
            data["random_id"] = random.randint(0, int(2e20))
    ...
    async def execute(self, code, **additional_values):
        ...
        async with self.session.post(f"https://api.vk.com/method/execute",
                data={"code": code, "access_token": self.token, "v": VERSION, "lang": 0,
    ...
    async def user(self, username, password, app_id, scope):
        ...
        self.logger.info("Access token: " + self.token)
