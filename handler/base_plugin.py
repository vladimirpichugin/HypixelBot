...
class CommandPlugin(BasePlugin):
    __slots__ = (
        "commands", "compiled_commands", "prefixes",
        "compiled_prefixes", "strict", "required_role"
    )

    def __init__(self, *commands, prefixes=None, strict=False, required_role="any", ):
        super().__init__()

        self.strict = strict
        self.required_role = required_role if required_role else "any"

        self.compiled_commands, self.commands = [], []   
        if commands:
    ...
    async def _check_message(self, msg):
        ...
        msg.meta["payload_cmd"] = None
        msg.meta["payload_act"] = None
        msg.meta["payload_obj"] = None
        
        if msg._payload:
            raw_payload = msg._payload if type(msg._payload) == dict else json.loads(msg._payload)
            
            if type(raw_payload) == dict:
                if "cmd" in raw_payload and raw_payload["cmd"]:
                    if type(raw_payload["cmd"]) == str:
                        if raw_payload["cmd"] == self.name:
                            msg.meta["payload_obj"] = raw_payload
                            msg.meta["payload_cmd"] = raw_payload["cmd"]  
                            
                            if "act" in raw_payload:
                                msg.meta["payload_act"] = raw_payload["act"]
