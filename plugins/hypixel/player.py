...
class HypixelPlayerPlugin(CommandPlugin):
    ...
    def __init__(self, *commands, prefixes=None, strict=False, required_role=None, game=None, timeout=120, ):
        ...
        self.description = ["Игрок Hypixel"]
        
        self.game = game
        self.timeout = timeout
        self.api_command = "stats"
        
        super().__init__(*commands, prefixes=prefixes, strict=strict, required_role=required_role)   
        
        example = self.command_example()
        if self.game:
            self.help_message = f"Статистика игрока в {self.game}\n{example} [ник/вк]"
        else:
            self.help_message = f"Статистика игрока\n{example} [ник/вк] [игра]"    
    ...
    async def process_message(self, msg):
        ...
        if self.game:
            if self.game == "SkyBlock":
                args.insert(1, self.game)
            else:
        ...
        await msg.answer("&#128640; Загрузка статистики игрока..")
