...
from utils import get_hypixel_api_error_message
from utils import upload_photo_by_file
from utils import create_report


class HypixelPlayerCSPlugin(CommandPlugin):
    __slots__ = ("description", "timeout", "api_command", "api_url", "api_headers", "help_message", "game", )
    
    def __init__(self, *commands, prefixes=None, strict=False, required_role=None, game=None, timeout=120, ):
        """Compare players stats command."""
        self.description = ["Сравнение статистики игроков Hypixel"]
        
        self.game = game
        self.timeout = timeout
        self.api_command = "compare"
        
        super().__init__(*commands, prefixes=prefixes, strict=strict, required_role=required_role)    
        
        example = self.command_example()
        if self.game:
            self.help_message = f"Сравнить статистику игроков в {self.game}\n{example} [1 ник/вк] [2 ник/вк]"
        else:
            self.help_message = f"Сравнить статистику игроков\n{example} [1 ник/вк] [2 ник/вк] [игра]"   
    ...
    async def process_message(self, msg):
        ...
        if len(args) < 1:
            return await msg.answer(self.help_message)

        if self.game:
            args.append(self.game)

        cmd_args = " ".join(args)
        
        await msg.answer("&#128640; Сравнение статистики..")
