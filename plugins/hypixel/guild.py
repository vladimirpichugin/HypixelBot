...
class HypixelGuildPlugin(CommandPlugin):
    __slots__ = ("description", "timeout", "api_command", "api_url", "api_headers", "help_message", )
    
    def __init__(self, *commands, prefixes=None, strict=False, required_role=None, timeout=120, ):
        """Guild stats command."""
        self.description = ["Гильдия Hypixel"]

        self.timeout = timeout        
        self.api_command = "guild"

        super().__init__(*commands, prefixes=prefixes, strict=strict, required_role=required_role)
        
        example = self.command_example()
        self.help_message = f"Статистика гильдии\n{example} [название гильдии]"
    ...
    async def process_message(self, msg):
        ...
        #detail = True if "-d" in cmd_args else False
        detail = True
        
        if detail:
            await msg.answer("&#128640; Загрузка статистики гильдии..\n\nЗагрузка подробной статистики может занять некоторое время.")
        else:
            await msg.answer("&#128640; Загрузка статистики гильдии..")
